...
// 全国天地图
const globalSubDomains = ['t0', 't1', 't2', 't3', 't4', 't5', 't6', 't7']

const config = {
  // 山东省天地图配置
  shandong: {
    tileInfo: tileInfoConfig.shandong,
  ...
  // 全国天地图配置
  global: {
    tileInfo: tileInfoConfig.global,
    urlTemplate: 'http://{subDomain}.tianditu.gov.cn/DataServer?T=vec_c&x={col}&y={row}&l={level}',
    subDomains: globalSubDomains
  },
  global_node: {
    tileInfo: tileInfoConfig.global,
    urlTemplate: 'http://{subDomain}.tianditu.gov.cn/DataServer?T=cva_c&x={col}&y={row}&l={level}',
    subDomains: globalSubDomains
  },
  global_water: {
    tileInfo: tileInfoConfig.global,
    urlTemplate: 'http://{subDomain}.tianditu.gov.cn/wat_c/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=wat&tileMatrixSet=c&TileMatrix={level}&TileRow={row}&TileCol={col}&style=default&format=tiles',
    subDomains: globalSubDomains
  },
  global_image: {
    tileInfo: tileInfoConfig.global,
    urlTemplate: 'http://{subDomain}.tianditu.gov.cn/img_c/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=img&tileMatrixSet=c&TileMatrix={level}&TileRow={row}&TileCol={col}&style=default&format=tiles',
    subDomains: globalSubDomains
  },
  global_terrain: {
    tileInfo: tileInfoConfig.global,
    urlTemplate: 'http://{subDomain}.tianditu.gov.cn/ter_c/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=ter&tileMatrixSet=c&TileMatrix={level}&TileRow={row}&TileCol={col}&style=default&format=tiles',
    subDomains: globalSubDomains
  }
}
// 全国天地图配置
const globalConfigs = ['global', 'global_node', 'global_water', 'global_image', 'global_terrain']
  ...
  methods: {
    ...
    getConfig: function () {
      if (this.preset) {
        ...
        if (globalConfigs.indexOf(this.preset) !== -1) {
